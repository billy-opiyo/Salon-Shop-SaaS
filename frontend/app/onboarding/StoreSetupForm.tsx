...
export function StoreSetupForm() {
  ...
  return (
    <form className="auth-form onboarding-form" onSubmit={handleSubmit}>
      ...
      <label>
        Store address
        <input name="slug" type="text" required minLength={3} maxLength={48} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="your-salon" />
      </label>
      <label>
        Starting plan
      ...
      <div className="onboarding-form__row">
        <label>City<input name="city" type="text" maxLength={80} placeholder="Nairobi" /></label>
        <label>Country<input name="country" type="text" defaultValue="Kenya" required maxLength={80} /></label>
      </div>
      ...
      <p className="auth-form__switch"><Link href="/manage">Return to workspace</Link></p>
    </form>
  );
}

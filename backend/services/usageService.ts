...
async function getTenantPlan(tenantId: string) {
	...
	const tier = (tenant?.subscription?.plan.tier.toLowerCase() ?? "starter") as keyof typeof PLAN_ENTITLEMENTS
...
export async function assertGalleryCapacity(tenantId: string) {
	...
	if (count >= plan.limits.galleryItems) throw new UsageLimitError("Your plan gallery limit has been reached.")
}

export async function getTenantUsage(tenantId: string) {
	const [plan, galleryItems, staffMembers, storage, bookings] = await Promise.all([
		getTenantPlan(tenantId),
		prisma.galleryStyle.count({ where: { tenantId } }),
		prisma.stylist.count({ where: { tenantId } }),
		prisma.mediaAsset.aggregate({ where: { tenantId }, _sum: { byteSize: true } }),
		prisma.booking.count({ where: { tenantId, createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } } }),
	])
	return {
		...
		storageMegabytes: { used: Math.ceil((storage._sum.byteSize ?? 0) / 1024 / 1024), limit: plan.limits.storageMegabytes },
